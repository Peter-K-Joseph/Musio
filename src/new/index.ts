...
class Player {
    ...
    updatePlayerInfo = (title, subtitle, album_art, restore) => {
        ...
        if (album_art != null) {
            (document.querySelector("#album_art") as HTMLDivElement).style.backgroundImage = `url(${album_art})`;
        }
    }
}
...
class errorManager {
    constructor(main: string, sub: string, traceback: any) {
...
const changeMainView = (nav: string) => {
