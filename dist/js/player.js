...
const syncTrack = () => {
    ...
    $.post(`${window.location}/../../server/song_list.json`)
        .done((val) => {
        ...
        const calc_duration = (stream) => {
            ...
            for (let i = 0; i < stream.length; i++) {
                (stream.length - 1 != i) ? (calc = calc + parseFloat(stream[i]) * 60) : (calc = calc + parseFloat(stream[i]));
            }
            return calc;
        };
        if (val.length == 0) {
...
const resync = () => {
    ...
    $.post(`${window.location}/../../server/index.php?resync`).done(() => {
